...
export default class DepartmentAIConfigList extends React.Component {
	...
	render() {
		const { data, total, orgOpts, departmentOpts } = this.props.departmentAiConfig;
		const { page, pageSize, showPopup, hotOne } = this.state;
		const columns = [
			{dataIndex:'department_name', title:'科室'},
			{dataIndex:'org_name', title:'机构'},
			{dataIndex:'content', title:'自动学习', render: this.contentRender.bind(this)},
		...
		return (
			<div style={{width:1000, margin:'auto', backgroundColor:'white', padding:20}}>
				...
				<Table columns={columns}
				       ...
				       pagination={pageOpts}
				/>
				<DepartmentAIConfigPopup visible={showPopup}
				                    item={hotOne}
