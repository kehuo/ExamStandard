...
export default class SymptomPanel extends React.Component {
	...
	buildParentTree() {
		const { parentTree } = this.state;
		let extra = (
			<Button type="primary" onClick={this.onShowNewPopup.bind(this, null)}>新建症状</Button>
		);
		return (
	...
	render() {
		const { page, pageSize, showPopup, data, total, hotOne,
			showNodePopup, parentItems, extensionOpts } = this.state;
		const columns = [
		...
		return (
			<div style={{width:1000, backgroundColor:'white', padding:10}}>
				...
				<Table columns={columns}
				       ...
				       pagination={pageOpts}
				/>
				<SymptomPopup visible={showPopup}
				                    item={hotOne}
				                    onSubmit={this.onSubmit.bind(this)}
				              extensionOpts={extensionOpts}
				/>
				<NodeSelectPopup visible={showNodePopup} title="症状"
				                 value={parentItems}
