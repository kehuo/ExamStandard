...
export default class BodyStructruePanel extends React.Component {
	...
	buildParentTree() {
		const { parentTree } = this.state;
		let extra = (
			<Button type="primary" onClick={this.onShowNewPopup.bind(this, null)}>新建人体器官</Button>
		);
		return (
	...
	render() {
		const { page, pageSize, showPopup, data, total, hotOne,
			showNodePopup, parentItems } = this.state;
		const columns = [
		...
		return (
			<div style={{width:1000, backgroundColor:'white', padding:10}}>
				...
				<Table columns={columns}
				       ...
				       pagination={pageOpts}
				/>
				<BodyStructurePopup visible={showPopup}
				                 item={hotOne}
				                 onSubmit={this.onSubmit.bind(this)}
				/>
				<NodeSelectPopup visible={showNodePopup} title="身体部位"
				                 value={parentItems}
