...
export default class SuggestFormatSetList extends React.Component {
	...
	buildOpBar() {
		...
		return (
			<Row style={{marginTop:10, marginBottom:10}}>
				...
				<Col span={4} offset={1}>
					<Button type="primary" onClick={this.onShowNewPopup.bind(this, null)}>新建机构配置</Button>
				</Col>
			</Row>
		);
	}
	...
	render() {
		const { data, total, orgOpts, contentOptions } = this.props.orgSuggestFormat;
		const { page, pageSize, showPopup, hotOne } = this.state;
		const columns = [
			{dataIndex:'org_name', title:'机构'},
			{dataIndex:'content', title:'推荐数据配置', render: this.contentRender.bind(this)},
		...
		return (
			<div style={{width:1000, margin:'auto', backgroundColor:'white', padding:20}}>
				...
				<Table columns={columns}
				       ...
				       pagination={pageOpts}
				/>
				<SuggestFormatPopup visible={showPopup}
				            item={hotOne}
