...
def getCurrentPage(db, page, pageSize, startId):
    ...
    try:
        ...
        rows = db.session.query(
            FMAEntity.id, FMAEntity.en_name
        ) \
            .filter(FMAEntity.id >= startId) \
            .order_by(FMAEntity.id) \
            .offset(offset).limit(pageSize).all()
        records = build_rows_result(rows, ['id', 'en_name'])
    except Exception as e:
        db.session.rollback()
        error = traceback.format_exc()
        print(error)
...
def updateRecords(newAs, db):
    try:
        validCnt = 0
        for one in newAs:
            record = db.session.query(FMAEntity).filter_by(id=one['id']).first()
            if record is None:
                print('bad {}'.format(json.dumps(one, ensure_ascii=False)))
                continue
            record.cn_name = one['cn_name']
            db.session.flush()
